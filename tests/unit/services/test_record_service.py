...
from src.services.service_base import NoConentException
...
def test_record_service_paginate_then_return_list():
    # arrange
    record = FakerRecord()
    record_repository_mock = Mock(RecordRepository)
    record_repository_mock.get_all.return_value = [record.model()]
    storage_mock = Mock(SimpleStorageService)

    record_service = RecordService(record_repository_mock, storage_mock)

    # act
    result = record_service.paginate()

    # assert
    record_repository_mock.get_all.assert_called_once()
    assert result


def test_record_service_paginate_then_raise_no_content_exception():
    # arrange
    record_repository_mock = Mock(RecordRepository)
    record_repository_mock.get_all.return_value = []
    storage_mock = Mock(SimpleStorageService)
...
def test_record_service_create_then_object():
    ...
    record = FakerRecord()
    record_repository_mock = Mock(RecordRepository)
    record_repository_mock.search_by_id.return_value = record.model()
    storage_mock = Mock(SimpleStorageService)
    ...
    # assert
    record_repository_mock.save.assert_called_once()
    assert result
